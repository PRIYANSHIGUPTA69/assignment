...
function createData(rowId,ItemName, ItemCode,Category,ItemType, PurchasePrice ,item,url,tax , description,stock) {
  console.log(ItemName)
return  {rowId,ItemName, ItemCode,Category,ItemType, PurchasePrice,item,url,tax,description,stock}
}
...
const headCells = [
  ...
  { id: 'quantity', numeric: false, disablePadding: false, label: 'Stock Quantity' },
    ...
    snapshot.forEach(doc => {
     ...
     let obj
      ...
      = {
        ...
        console.log(obj)
        const arr = [
            createData(obj.rowId,obj.ItemName , obj.ItemCode , obj.Category , "Product",obj.PurchasePrice,false,obj.url,obj.tax,obj.description,obj.stockQuantity )
        ]
        rows = [...rows,arr[0]];
        console.log(rows)
   });
                ...
                .map((row, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <ItemDetails row={row} type={row.item} Rid={row.rowId} text={row.Category}></ItemDetails></TableCell>
                      <TableCell align="left">
                      <ItemDetails row={row} type={row.item} Rid={row.rowId} text={row.stock}></ItemDetails>
                      </TableCell>
                      <TableCell align="left">
                      <ItemDetails row={row} type={row.item} Rid={row.rowId} text={row.PurchasePrice}></ItemDetails>
                      </TableCell>
                      <TableCell align="left"><EditModal inventory={true} Rid={row.rowId}></EditModal></TableCell>
                    </TableRow>
                  );
                })}
